...
router = APIRouter(
    prefix="/employee_shift_assignment", tags=["employee_shift_assignment"]
)
...
@router.get(
    ...
    response_model=list[EmployeeShiftAssignmentResponse],
)
async def get_employee_shift_assignments(db: db_dependency):
    employee_shift_assignments = (
        EmployeeShiftAssignmentService.get_employee_shift_assignments(db)
    )
...
async def get_employee_shift_assignment(
    db: db_dependency, employee_shift_assignment_id: int = Path(gt=0)
):
    employee_shift_assignment = (
        EmployeeShiftAssignmentService.get_employee_shift_assignment_by_id(
            db, employee_shift_assignment_id
        )
    )
...
@router.post(
    ...
    response_model=EmployeeShiftAssignmentResponse,
)
async def create_employee_shift_assignment(
    db: db_dependency, employee_shift_assignment_request: EmployeeShiftAssignmentRequest
):
    new_employee_shift_assignment = (
        EmployeeShiftAssignmentService.create_employee_shift_assignment(
            db, employee_shift_assignment_request
        )
    )
    if new_employee_shift_assignment:
        return new_employee_shift_assignment
    raise HTTPException(
        status_code=400, detail="Failed to create a new employee_shift_assignment."
    )


@router.put(
    "/", status_code=status.HTTP_200_OK, response_model=EmployeeShiftAssignmentResponse
)
async def update_employee_shift_assignment(
    db: db_dependency, employee_shift_assignment_request: EmployeeShiftAssignmentRequest
):
    updated_employee_shift_assignment = (
        EmployeeShiftAssignmentService.update_employee_shift_assignment(
            db, employee_shift_assignment_request
        )
    )
    ...
    raise HTTPException(
        status_code=404, detail="EmployeeShiftAssignment not found or failed to update."
    )


@router.delete(
    "/{employee_shift_assignment_id}", status_code=status.HTTP_204_NO_CONTENT
)
async def delete_employee_shift_assignment(
    db: db_dependency, employee_shift_assignment_id: int = Path(gt=0)
):
    success = EmployeeShiftAssignmentService.delete_employee_shift_assignment(
        db, employee_shift_assignment_id
    )
    if not success:
        raise HTTPException(
            status_code=404,
            detail="EmployeeShiftAssignment not found or failed to delete.",
        )
