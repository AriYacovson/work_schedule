...
async def update_employee(db: db_dependency,
                          ...
                          employee_id: int = Path(gt=0)):
    updated_employee = EmployeeService.update_employee(db, employee_id, employee_request)
    if updated_employee:
        return updated_employee
    raise HTTPException(status_code=404, detail="Employee not found or failed to update.")
...
async def delete_employee(db: db_dependency,
    ...
    if not success:
        raise HTTPException(status_code=404, detail="Employee not found or failed to delete.")
